...
`icon` renders any element before the text: pass a `TVajraIconComponent` from `@devraj-labs/vajra-ui-icons` (or your own). `dismissIcon` replaces the default ✕ glyph on the dismiss button.
