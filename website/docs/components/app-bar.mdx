...
import { ChevronLeftIcon, SettingsIcon, SearchIcon, HeartIcon } from '@devraj-labs/vajra-ui-icons';

# App Bar

A screen header built from composable slots: `AppBar.Header`, `AppBar.Title`, `AppBar.BackAction`, and `AppBar.IconButton`. Left, center, and right content are detected automatically from the children you pass in. Both icon slots accept any component matching `TVajraIconComponent`; the examples below use [`@devraj-labs/vajra-ui-icons`](https://github.com/devraj-labs/vajra-ui-icons), a small set of Lucide icons pre-adapted to that contract.
