...
A screen header built from composable slots — `AppBar.Header`, `AppBar.Title`, `AppBar.BackAction`, and `AppBar.IconButton`. Left, center, and right content are detected automatically from the children you pass in.

export const DotIcon = ({ size, color }) => (
  <View style={{ width: size, height: size, borderRadius: size / 2, backgroundColor: color }} />
);

export const ChevronIcon = ({ size, color }) => (
  <View
    style={{
      width: size * 0.5,
      height: size * 0.5,
...
<ComponentPreview>
  <AppBar.Header>
    <AppBar.BackAction icon={ChevronIcon} onPress={() => {}} />
    <AppBar.Title>Profile</AppBar.Title>
    <AppBar.IconButton icon={DotIcon} onPress={() => {}} accessibilityLabel="More" />
  </AppBar.Header>
</ComponentPreview>
