...
A small, non-interactive label built on `Box`, typically used for counts, statuses, or tags. Accepts an optional leading icon: any component matching `TVajraIconComponent`. The example below uses [`@devraj-labs/vajra-ui-icons`](https://github.com/devraj-labs/vajra-ui-icons), a small set of Lucide icons pre-adapted to that contract.
