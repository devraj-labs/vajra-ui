...
import { StarIcon, CheckCircleIcon, AlertCircleIcon } from 'vajra-ui-icons';

# IconBox

A fixed-size, token-driven container for displaying a single icon, typically used as a leading visual in list rows or feature callouts. Vajra UI ships no bundled icons: `icon` accepts any component matching `TVajraIconComponent`. The examples below use [`vajra-ui-icons`](https://github.com/devraj-labs/vajra-ui-icons), a small set of Lucide icons pre-adapted to that contract.
