...
import { SunIcon, MoonIcon } from '@devraj-labs/vajra-ui-icons';

# IconSwitch

A two-state sliding switch that shows a distinct icon in each of its "off" and "on" positions. Vajra UI ships no bundled icons: `offIcon`/`onIcon` accept any component matching `TVajraIconComponent`. The examples below use [`@devraj-labs/vajra-ui-icons`](https://github.com/devraj-labs/vajra-ui-icons), a small set of Lucide icons pre-adapted to that contract.
